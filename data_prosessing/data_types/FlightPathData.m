classdef FlightPathData
    properties
        ...
        DeltaESp
        DeltaRSp
        DeltaASpAbsolute % Values without trim subtracted
        DeltaESpAbsolute % Values without trim subtracted
        C_X
        C_Y
        C_Z
        C_L
    ...
    methods
        ...
        function obj = FlightPathData(id, maneuver_type)
            ...
            obj.Trim.DeltaE = delta_e_nom;
            obj.Trim.DeltaA = delta_a_nom;
            obj.Trim.Alpha = alpha_nom;
            obj.Trim.V = V_nom;
        end
        function obj = calc_fpr_from_rawdata(obj, dt_desired, dt_knots)
            ...
            % correct time frame
            obj.DeltaASpAbsolute = interp1(obj.RawData.TimeInput, obj.RawData.DeltaASp, obj.Time);
            obj.DeltaASp = obj.DeltaASpAbsolute - obj.Trim.DeltaA;
            obj.DeltaESpAbsolute = interp1(obj.RawData.TimeInput, obj.RawData.DeltaESp, obj.Time);
            obj.DeltaESp = obj.DeltaESpAbsolute - obj.Trim.DeltaE;
            ...
            obj.Alpha = obj.AlphaAbsolute - obj.Trim.Alpha;
...
%            ylim([-])
        ...
        function plot_lateral_validation(obj, t_sim, simulated_responses, model_names, plot_styles, show_plot, save_plot, filename, plot_location)
            ...
            subplot(num_plots_rows,2,1)
            plot(obj.Time, obj.VelV, plot_style_recorded_data); hold on;
            for i = 1:num_models
            ...
            subplot(num_plots_rows,2,3)
            plot(obj.Time, rad2deg(obj.AngP), plot_style_recorded_data); hold on
            for i = 1:num_models
            ...
            subplot(num_plots_rows,2,5)
            plot(obj.Time, rad2deg(obj.AngR), plot_style_recorded_data); hold on
            for i = 1:num_models
            ...
            subplot(num_plots_rows,2,7)
            plot(obj.Time, rad2deg(obj.EulPhi), plot_style_recorded_data); hold on
            for i = 1:num_models
        ...
        function plot_longitudinal_validation(obj, t_sim, simulated_responses, model_names, plot_styles, show_plot, save_plot, filename, plot_location)
            ...
            for i = 1:num_models
                plot(t_sim, simulated_responses.(model_names(i))(:,2), plot_styles(i)); hold on
            end
            %legend(model_names);
            ylabel("[m/s]")
            ylim([-10 10])
