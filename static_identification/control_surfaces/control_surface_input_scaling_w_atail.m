...
aileron_max_deg = 25;
elevator_max_deg = 25;
% Note I am actually not sure what the MAX deflection angle for a single
% a-tail control surface is. For now, I assume that this is 25 degrees,
% same as aileron.
rudder_max_deg = 22;
...
% Rudder input
figure
left_a_tail_deg = [90, 96, 103, 110, 112] - kDEG_OFFSET; % Measured a-tail angles
actual_px4_inputs_rudder = inputs + trim_yaw;

plot(actual_px4_inputs_rudder, left_a_tail_deg, '-o'); hold on
yline(rudder_max_deg);
xline(1);

linear_coeffs_yaw = least_squares_est(...
    [ones(1,N_data_points); actual_px4_inputs_rudder(1:end-1)], left_a_tail_deg(1:end-1)'...
    );
...
% Elevator input
figure
left_a_tail_angle_deg = [90, 97, 105, 113, 115] - kDEG_OFFSET;  % Measured elevator angles
actual_px4_inputs_elevator = inputs + trim_pitch;

plot(actual_px4_inputs_elevator, left_a_tail_angle_deg, '-o'); hold on
yline(elevator_max_deg);
xline(1);

linear_coeffs_pitch = least_squares_est(...
    [ones(1,N_data_points); actual_px4_inputs_elevator(1:end-1)], left_a_tail_angle_deg(1:end-1)'...
    );

offset_elevator = linear_coeffs_pitch(1)
linear_term_elevator = linear_coeffs_pitch(2)
...
linear_coeffs_roll = least_squares_est(...
    [ones(1,N_data_points); actual_px4_inputs_aileron(1:end-1)], left_aileron_angle_deg(1:end-1)'...
    );

offset_aileron = linear_coeffs_roll(1)
linear_term_aileron = linear_coeffs_roll(2)
