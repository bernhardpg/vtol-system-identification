function cost = cost_fn_lon(x, dt, t_seq, y_lon_seq, y_lat_seq, input_seq, const_params, maneuver_indices)
    ...
    for i = 1:length(maneuver_indices)-1
        ...
        y0 = y_lon_seq_m(1,:);
        [t_pred, y_lon_pred] = ode45(@(t,y) aircraft_dynamics_lon(t, y, t_seq_m, input_seq_m, y_lat_seq_m, all_params), tspan, y0);
        y_lon_pred = interp1(t_pred, y_lon_pred, t_seq_m);
        
        % Squared cost
        costs(i) = sum(diag((y_lon_seq_m - y_lon_pred)' * (y_lon_seq_m - y_lon_pred)));
    end
    cost = sum(costs);
end
